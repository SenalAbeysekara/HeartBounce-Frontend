...
export default function PlayerProfile() {
  ...
  // ✅ Fallback stats computed from runs (always correct)
  const computedStats = useMemo(() => {
    ...
    for (const r of runs) {
      ...
      if (r.score > base[r.difficulty].bestScore) base[r.difficulty].bestScore = r.score;
    }
    return base;
  }, [runs]);

  // ✅ Use backend stats if valid, else fallback
  const finalStats = useMemo(() => {
    const s = diffStats || computedStats;

    // if backend returns all zeros but you have runs, use fallback
    const sumRuns = (s?.easy?.runs || 0) + (s?.medium?.runs || 0) + (s?.hard?.runs || 0);
    ...
    <AuthLayout>
      <div className="w-full max-w-4xl rounded-3xl border border-white/10 bg-white/10 p-8 backdrop-blur-2xl shadow-2xl">
        <div className="flex items-center justify-between">
          ...
          <Link to="/mainmenu" className="text-white/70 hover:text-white">← Back</Link>
        </div>
              ...
              <div className="space-y-3">
                ...
                <div className="flex gap-3">
                  <button
                    ...
                    className={`rounded-xl bg-linear-to-br from-fuchsia-500 via-indigo-500 to-cyan-400 px-5 py-2 text-sm font-extrabold text-[#070a18] hover:brightness-110 transition ${saving ? "opacity-60 cursor-not-allowed" : ""
                      }`}
                  >
                    {saving ? "Saving..." : "Save"}
                  </button>
